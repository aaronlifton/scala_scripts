...
object Tour extends App {
  ...
  def dim(line: String): Either[Boolean, Double] = {
    ...
    catch { case e: Throwable ⇒ Left(false) }
  }
  ...
  "3\n1 3 5\n2 4 6\n3 4.5 7".split("\n").foreach { line ⇒
    if (line.length == 1) {
      dims += dim(line)
      paths(dim(line) match { case Right(i: Double) ⇒ i.toInt }) = ListBuffer[List[Double]]()
    } else {
  ...
  println(paths.map(_._2).map(_.transpose))
  ...
  val sums: mutable.Iterable[Seq[Double]] = {
    paths.map(_._2.map(_.foldRight(0.0)(_ + _)))
  }
